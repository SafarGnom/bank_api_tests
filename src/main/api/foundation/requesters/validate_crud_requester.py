...
class ValidateCrudRequester(HttpRequester):
    ...
    def post(self,model: Optional[BaseModel] = None) -> BaseModel:
        ...
        with allure.step(f"POST {Config.fetch("backendUrl")}{self.endpoint.value.url} and Validate Model"):
            allure.attach(f"Validate Model response: {self.endpoint.value.response_model.__name__}")

        self.response_spec(response)

        # Если получили ошибку 4xx/5xx —
        # не пытаемся валидировать её как успешную модель
